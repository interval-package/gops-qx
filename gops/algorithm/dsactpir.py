...
__all__=["ApproxContainer","DSACTPIR"]
import time
...
class DSACTPIR(AlgorithmBase):
    ...
    def __compute_gradient(self, data: DataDict, iteration: int):
        ...
        loss_q, q1, q2, std1, std2, min_std1, min_std2, loss_q_comp , q1_comp, q2_comp, origin_q_loss = self.__compute_loss_q(data)
        loss_q.backward()
        loss_q_comp.backward()
        ...
        tb_info = {
            ...
            "DSAC2/loss_critic_comp": loss_q_comp.item(),
    ...
    def __compute_loss_q_comp(self, data: DataDict, act2: torch.Tensor) -> torch.Tensor:
        obs, _, _, obs2, done = (
            data["obs"],
            data["act"],
            data["rew"],
        ...
        reward_comp = data["next_reward_comps"]*self.reward_comp_factor
        q1_comp = self.networks.q1.cal_comp(obs, act2)
        q2_comp = self.networks.q2.cal_comp(obs, act2)
        with torch.no_grad():
            q1_next_comp = self.networks.q1_target.cal_comp(obs2, act2)
            q2_next_comp = self.networks.q2_target.cal_comp(obs2, act2)
            q_next_comp = torch.min(q1_next_comp, q2_next_comp)
        target_q1_comp = reward_comp + (1 - done.unsqueeze(-1)) * self.gamma * q_next_comp
        target_q2_comp = reward_comp + (1 - done.unsqueeze(-1)) * self.gamma * q_next_comp
        loss_q1_comp = ((q1_comp - target_q1_comp) ** 2).mean()
        loss_q2_comp = ((q2_comp - target_q2_comp) ** 2).mean()
        loss_q_comp = loss_q1_comp + loss_q2_comp
        return loss_q_comp, q1_comp, q2_comp
    ...
    def __compute_loss_q(self, data: DataDict):
        ...
        loss_q_comp, q1_comp, q2_comp = self.__compute_loss_q_comp(data, act2)
        with torch.no_grad():
        ...
        return q1_loss +q2_loss, q1.detach().mean(), q2.detach().mean(), q1_std.detach().mean(), q2_std.detach().mean(), q1_std.min().detach(), q2_std.min().detach(), loss_q_comp, q1_comp, q2_comp, origin_q_loss.detach()
