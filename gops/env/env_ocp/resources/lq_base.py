...
class LqEnv(PythBaseEnv):
    ...
    def render(self, mode="human"):
        ...
        x_action = range(max(self.step_counter, 1))
        if self.step_counter == 0:
            y_action = np.zeros((1, self.action_dim))  # dummy action for plotting
        else:
        ...
        if self.first_rendering:
            ...
            for idx_a in range(self.action_dim):
                ...
                (line,) = ax.plot(x_action, y_action[:, idx_a], color="b")
                self.lines.append(line)
                self.axes.append(ax)
            ...
            for idx_s in range(self.observation_dim):
                ...
                self.axes[idx_s].set_ylim([y_state[:, idx_s].min() - 1, y_state[:, idx_s].max() + 1])

            for idx_a in range(self.action_dim):
                self.lines[self.observation_dim + idx_a].set_xdata(x_action)
                self.lines[self.observation_dim + idx_a].set_ydata(y_action[:, idx_a])
                ...
                self.axes[self.observation_dim + idx_a].set_ylim([y_action[:, idx_a].min() - 1, y_action[:, idx_a].max() + 1])
