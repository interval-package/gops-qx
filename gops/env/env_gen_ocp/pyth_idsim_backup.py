...
class CloudServer:
    ...
    def init_idsim(self, env_config: Config, model_config: ModelConfig, scenario_id: str):
        ...
        self.env = LasvsimEnv(**qianxing_config,env_config=env_config_dict,model_config=model_config_dict)
    ...
    def reset_idsim(self, options: List[dict] = None):
        return self.env.reset(expect_direction="right", options=options)
    ...
    def get_model_free_reward_details(self, idsim_context: BaseContext, last_last_action, 
        ...
        return self.env.model_free_reward(idsim_context, last_last_action, 
                                      last_action, action)
        # message = pickle.dumps((idsim_context,last_last_action,last_action, action))
        # request = cloudserver_pb2.GetModelFreeRewardDetailsRequest(
        #     message = message,
        #     idsim_id = self.idsim_id
        # )
        # response = self.stub.GetModelFreeRewardDetails(request)
        # message = pickle.loads(response.message)
        # return message
        ...
        if self.lc_cooldown_counter > self.lc_cooldown:
            ...
            if self.use_random_ref_param and np.random.rand() < self.random_ref_probability :
                ...
                if new_ref_index != self.ref_index:
                    ...
                    self.lc_cooldown_counter = 0
        reward_model, reward_details, next_state = self._get_reward(action)
