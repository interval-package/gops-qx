...
class ContextState(Generic[stateType]):
    ...
    def requires_grad_(self, requires_grad: bool = True) -> 'ContextState[torch.Tensor]':
        value = []
        for field in fields(self):
            v = getattr(self, field.name)
            if isinstance(v, torch.Tensor) and v.dtype.is_floating_point:
                value.append(v.requires_grad_(requires_grad))
            else:
                value.append(v)
        return self.__class__(*value)
...
class State(Generic[stateType]):
    ...
    def requires_grad_(self, requires_grad: bool = True) -> 'State[torch.Tensor]':
        assert isinstance(self.robot_state, torch.Tensor)
        robot_state = self.robot_state.requires_grad_(requires_grad)
        context_state = self.context_state.requires_grad_(requires_grad)
        return self.__class__(robot_state, context_state)
